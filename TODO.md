...
- (MEDIUM) re-enable skipped E2E test `multi-session.spec.ts` › "should show real-time updates when one user edits and another watches". It's skipped because it asserts a *viewer* receiving cross-client onSnapshot updates after the editor acts, and that propagation latency is too variable on shared staging Firebase to be reliable. The right fix is to run E2E against the Firebase emulator in CI (isolated, deterministic, ephemeral) instead of against staging — the emulator wiring already exists (`npm run test:e2e:emulator`), it just isn't a CI job. Attempting to make the editor-side waits deterministic on staging (#8) regressed the real-network bid-save timing and broke `bidding-flow`, so it was reverted; don't retry that approach against staging.
